...
#define AV_CURL_MAX_RETRIES		3
#define AV_CURL_RETRY_BASE_MS	500
#define AV_SMOOTH_BUFFER_SIZE	SCE_KERNEL_8MiB
#define AV_BOOT_READY_TIMEOUT_MS	30000
...
AVPlayer::SmoothStreaming::SmoothStreaming()
{
	...
	m_tmpBuf = static_cast<uint8_t *>(sce_paf_malloc(AV_SMOOTH_BUFFER_SIZE));
	sce_paf_memset(m_tmpBuf, 0, AV_SMOOTH_BUFFER_SIZE);
}
...
size_t AVPlayer::SmoothStreaming::CurlDownload(char *buffer, size_t size, size_t nitems, void *userdata)
{
	...
	if (toWrite != 0)
	{
		if (obj->m_total + toWrite >= AV_SMOOTH_BUFFER_SIZE)
		{
			SCE_DBG_LOG_ERROR("[CurlDownload] tmp buffer is too small! (%u + %u >= %u)", obj->m_total, toWrite, AV_SMOOTH_BUFFER_SIZE);
			return 0;
		}
	...
	else
	{
		...
		if (m_total == 0 && !m_downloadDone)
		{
			if (m_vod.get())
			{
				...
				if (m_total > AV_SMOOTH_BUFFER_SIZE)
				{
					SCE_DBG_LOG_ERROR("[LiveGetImpl] tmp buffer is too small! (%u > %u)", m_total, AV_SMOOTH_BUFFER_SIZE);
					m_lastError = 404;
					return -1;
				}
			...
			else
			{
				...
				for (int attempt = 0; attempt < AV_CURL_MAX_RETRIES; attempt++)
				{
					if (attempt > 0)
					{
						SCE_DBG_LOG_WARNING("[LiveGetImpl] Retry attempt %d/%d", attempt + 1, AV_CURL_MAX_RETRIES);
						m_total = 0;
						thread::Sleep(AV_CURL_RETRY_BASE_MS * (1 << attempt));
					}
				...
				if (res != CURLE_OK)
				{
					SCE_DBG_LOG_ERROR("[LiveGetImpl] curl_easy_perform() returned %d after %d attempts, status %d", res, AV_CURL_MAX_RETRIES, m_lastError);
					return -1;
				}
			}
...
void AVPlayer::OnBootJob()
{
	...
	*/
	...
	// Wait for stream to become ready with a timeout
	uint32_t bootWaitMs = 0;
	while (m_currentState != PlayerState_Ready)
	{
		thread::Sleep(100);
		bootWaitMs += 100;
		if (bootWaitMs >= AV_BOOT_READY_TIMEOUT_MS)
		{
			SCE_DBG_LOG_ERROR("[BootJob] Timed out waiting for stream ready after %u ms", bootWaitMs);
			SetInitState(InitState_InitFail);
			return;
		}
	}
