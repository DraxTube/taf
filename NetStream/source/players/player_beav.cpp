...
#define BEAV_MAX_DOWNLOAD_SIZE	(SCE_KERNEL_64KiB * 1024)	// 64 MiB safety cap
#define BEAV_CURL_MAX_RETRIES	3
#define BEAV_CURL_RETRY_BASE_MS	500
...
size_t BEAVPlayer::LibLSInterface::DownloadCore(char *buffer, size_t size, size_t nitems, void *userdata)
{
	...
	if (toCopy != 0) {
		...
		if (obj->m_pos + toCopy > BEAV_MAX_DOWNLOAD_SIZE)
		{
			SCE_DBG_LOG_ERROR("[LS::DownloadCore] Buffer limit exceeded (%u + %u > %u)", obj->m_pos, toCopy, BEAV_MAX_DOWNLOAD_SIZE);
			return 0;
		}
		void *newBuf = sce_paf_realloc(obj->m_buf, obj->m_pos + toCopy);
		if (!newBuf)
		{
			SCE_DBG_LOG_ERROR("[LS::DownloadCore] realloc failed for size %u", obj->m_pos + toCopy);
			return 0;
		}
		obj->m_buf = newBuf;
...
// Helper: check if URI path (ignoring query string) ends with a given extension
static bool UriHasExtension(const char *uri, const char *ext)
{
	if (!uri || !ext)
		return false;

	// Find the path portion: stop at '?' or '#'
	const char *pathEnd = uri;
	const char *q = sce_paf_strchr(uri, '?');
	const char *h = sce_paf_strchr(uri, '#');
	const char *end = NULL;
	...
	// Find last '.' before query/fragment (or end of string)
	const char *dot = NULL;
	const char *scan = uri;
	while (*scan && scan != end)
	{
	...
	size_t extLen = sce_paf_strlen(ext);
	size_t availLen = end ? (size_t)(end - dot) : sce_paf_strlen(dot);
	if (availLen < extLen)
		return false;

	return (sce_paf_strncasecmp(dot, ext, extLen) == 0);
}
...
LSInputResult BEAVPlayer::LibLSInterface::Open(char *pcURI, uint64_t uOffset, uint32_t uTimeOutMSecs)
{
	...
	// Query-string-aware extension detection for HLS URIs
	if (UriHasExtension(pcURI, ".m3u8") || UriHasExtension(pcURI, ".m3u"))
	{
		m_curl = s_plCurl;
		lockMtx = &s_plMtx;
	}
	else if (UriHasExtension(pcURI, ".ts") || UriHasExtension(pcURI, ".aac") || UriHasExtension(pcURI, ".mp4"))
	{
		m_curl = s_fileCurl;
		lockMtx = &s_fileMtx;
	}
	...
	for (int attempt = 0; attempt < BEAV_CURL_MAX_RETRIES; attempt++)
	{
		// Reset download buffer for retry
		if (attempt > 0)
		{
			if (m_buf)
			{
			...
			m_pos = 0;
			m_readPos = 0;
			SCE_DBG_LOG_WARNING("[LS::Open] Retry attempt %d/%d for %s", attempt + 1, BEAV_CURL_MAX_RETRIES, pcURI);
			thread::Sleep(BEAV_CURL_RETRY_BASE_MS * (1 << attempt));
		}
	...
	if (ret != CURLE_OK)
	{
		SCE_DBG_LOG_ERROR("[LS::Open] curl_easy_perform returned %d after %d attempts", ret, BEAV_CURL_MAX_RETRIES);
		m_lastError = ret;
...
static int32_t s_lsErrorCount = 0;
#define BEAV_LS_MAX_ERRORS 5
...
LSResult BEAVPlayer::LsErrorHandler(LSResult error, LSSession *session, LSStreamlist *streamlist, LSStreamfile *streamfile, void *userdata)
{
	...
	{
		s_lsErrorCount++;
		SCE_DBG_LOG_ERROR("[LsErrorHandler] Error %d (count: %d/%d)", error, s_lsErrorCount, BEAV_LS_MAX_ERRORS);

		// Only fail after multiple consecutive errors to tolerate transient issues
		if (s_lsErrorCount >= BEAV_LS_MAX_ERRORS)
		{
			SCE_DBG_LOG_ERROR("[LsErrorHandler] Max errors reached, failing");
...
LSResult BEAVPlayer::LsInitGlobalsHook(LSLibraryInitParams *params)
{
	//params->uOptionFlags = ;
	//params->uOptionFlags |= ;
	params->uNoContentTimeOutSecs = 20;
	params->uNoConnectionTimeOutSecs = 20;
	params->uM3u8RetrySleepMSecs = 1000;
	params->uM3u8RetryTimeOutMSecs = 30000;
	params->uSegmentMaxRetryCount = 15;
	...
	params->uMaxM3U8FileSize = SCE_KERNEL_2MiB;
	params->uM3U8WorkingMemSize = SCE_KERNEL_1MiB;
...
void BEAVPlayer::PreInit()
{
	...
	sceBeavCorePlayerLsHttpSetResolveConfig(2000, 5);
	sceBeavCorePlayerLsSetTimeout(10000, 10000);
