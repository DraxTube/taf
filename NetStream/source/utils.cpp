...
static size_t CurlDownloadCore(char *buffer, size_t size, size_t nitems, void *userdata)
{
	...
	if (toCopy != 0)
	{
		...
		if (!newBuf)
		{
			SCE_DBG_LOG_ERROR("[CurlDownloadCore] realloc failed for size %u", obj->m_pos + toCopy);
			return 0;
		}
		obj->m_buf = newBuf;
	...
	int32_t *pRespCode, void*(*liballoc)(size_t), void(*libdealloc)(void *), void*(*librealloc)(void *, size_t))
{
	...
	for (int attempt = 0; attempt < 3; attempt++)
	{
		if (attempt > 0)
		{
			SCE_DBG_LOG_WARNING("[DoGETRequest] Retry attempt %d/3 for %s", attempt + 1, url);
			if (ctx.m_buf)
			{
		...
		// Only retry on transient cURL errors
	...
	size_t *pRespBufSize, int32_t *pRespCode, void*(*liballoc)(size_t), void(*libdealloc)(void *), void*(*librealloc)(void *, size_t))
{
	...
	for (int attempt = 0; attempt < 3; attempt++)
	{
		if (attempt > 0)
		{
			SCE_DBG_LOG_WARNING("[DoPOSTRequest] Retry attempt %d/3 for %s", attempt + 1, url);
			if (ctx.m_buf)
			{
				sce_paf_free(ctx.m_buf);
				ctx.m_buf = NULL;
